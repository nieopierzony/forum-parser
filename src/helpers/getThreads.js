...
module.exports = async (forum, page = 1) => {
  ...
  $('.structItem').each((i, el) => {
    const metaDiv = el.children.find(c => c.attribs.class.includes('structItem-cell--meta'));
    const answers = +metaDiv.children[0].children[1].children[0].data;
    ...
    const isSticky =
      ...
      mainDiv.children[0].children.some(j =>
        j.children.find(f => f.children && f.children.find(l => l.data === 'Закреплено')),
      );
    ...
    const titleDiv = mainDiv.children
      ...
      .children.find(j => !j.attribs.class.includes('label'));
    ...
    threads.push({ id, title, author: { nick: authorNick, id: authorID }, createDate, isSticky, answers });
  });
